      ...
      setEvents(events.map(event =>
        event.id === selectedEvent.id
          ? {
              ...
              budget: event.budget - newExpense, // Subtract expense from budget
              expenseDetails: [
                ...(event.expenseDetails || []),
                { description: expenseDescription, amount: newExpense },
              ],
            }
          : event
      ));
            ...
            {upcomingEvents.map(event => (
              <Card key={event.id}>
                ...
                <CardContent>
                  ...
                  {event.expenseDetails && event.expenseDetails.length > 0 && (
                    <div className="mt-4">
                      <strong>Expense Descriptions:</strong>
                      <ul className="list-disc list-inside mt-2 text-sm text-muted-foreground">
                        {event.expenseDetails.map((detail, index) => (
                          <li key={index}>
...
export function EventSorcererComponent() {
  ...
  if (selectedEvent && newExpense !== null && newExpense > 0) {
    setEvents(events.map(event =>
      event.id === selectedEvent.id
        ? { ...event, expenses: event.expenses + newExpense }
        : event
    ));
    setNewExpense(null); // Reset the input field
  }
};
                      ...
                      <TabsContent value="budget">
                          <div className="space-y-4">
                              ...
                              <div className="flex items-center space-x-2">
                                  ...
                                  />
                                  <Button
                                      onClick={(e) => setNewExpense(parseFloat(e.target.value) || null)}
                                      />
                                      <Button onClick={handleAddExpense}>Add Expense</Button>
                              </div>
                          </div>
